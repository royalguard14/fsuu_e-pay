...
public function admin()
{
    ...
    // Number of Students this Academic Year
    $totalStudents = EnrollmentHistory::where('academic_year_id', $currentAcademicYear->id)->count();
    // Data for the grade-level graph
    ...
    foreach ($grades as $grade) {
        ...
        if ($fee) {
            ...
            $otherFees = collect(json_decode($fee->other_fees, true))->sum();
            $totalFeePerStudent = $fee->tuition_fee + $otherFees;
            $expectedCollections[] = $studentsCount * $totalFeePerStudent;
        } else {
            $expectedCollections[] = 0;
        }
        // Total collected for this grade
   ...
   // Data for line chart: total collected money grouped by month
   //  $monthlyCollections = Payment::whereHas('enrollmentHistory', function ($query) use ($currentAcademicYear) {
   //          $query->where('academic_year_id', $currentAcademicYear->id);
   //      })
   ...
   //      ->orderBy('month')
   //      ->get();
   //  $months = $monthlyCollections->pluck('month');
   //  $monthlyTotals = $monthlyCollections->pluck('total');

    // Data for line chart: total collected money grouped by day
    ...
    return view('dashboard.admin', compact('totalExpected', 'totalCollected', 'totalStudents', 'currentAcademicYear', 'gradeLabels', 'expectedCollections', 'collectedMoney', 'days','dailyTotals'));
}
}